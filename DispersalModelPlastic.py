...
reefs_csv = "/home/jwren/Dropbox/School/AccessoryFiles/HIreefsNew.csv"
currents_csv = "/home/jwren/Dropbox/School/AccessoryFiles/files_HA8km.csv"
currents_u_suffix = "_100m_u.txt"
currents_v_suffix = "_100m_v.txt"
currents_data_dir = "/media/jwren/OfficeHD/HYCOM/globalHYCOM/100m/"
...
habilon = [0.0]*(releasesites + 1) # 302
habilat = [0.0]*(releasesites + 1) # 302
island = [0.0]*(releasesites + 1) # 302 

#### LOADING SETTLEMENT AND RELEASE HABITAT FROM FILE ####
print "Reading coastal fringe location file..."
with open ("HIreefs.csv", "r") as file3:
    for ijk in xrange(1, releasesites + 1):
        habilat[ijk], habilon[ijk], island[ijk] = map(float, file3.readline().rstrip().split(","))
