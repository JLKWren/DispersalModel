...
def checkdepth(xlon, xlat, currentday, uval, vval):
    ...
    if 1 <= nnlon <= londim and 1 <= nnlat <= latdim:
        ...
        if hycomu == EVEL_NVEL_ERR or hycomv == EVEL_NVEL_ERR:
            #print >> file9,  "%s\t%s\t%s\t%s\t%s" % (xlon, xlat, hycomu, hycomv, startday)
...
with open ("/home/jwren/Dropbox/School/AccessoryFiles/HIreefsNew.csv", "r") as file3:
    for ijk in xrange(1, releasesites + 1):
...
print "Reading daily currents..."
with open("/home/jwren/Dropbox/School/AccessoryFiles/files_HA8km.csv", "r") as file1:
    for ijk in xrange(122, 2038 + 1):
        line = file1.readline()
        xfile = line.split(",")[0]
...
for ijk in xrange(arraystart, arrayend + 1):
    print ijk
    ufile = "/media/jwren/HomeHD/HYCOM/globalHYCOM/100m/" + myFile[ijk] + "_100m_u.txt"
    vfile = "/media/jwren/HomeHD/HYCOM/globalHYCOM/100m/" + myFile[ijk] + "_100m_v.txt"
    if(not exists(ufile)):
...
outputfile = "/home/jwren/Desktop/HYCOM100m8kmRun2/HYCOM45pld50n100m_1.txt"
with open(outputfile, 'w') as file2 :
    #for idx, duration in enumerate(PLD):
        for startsite in xrange(1, releasesites + 1):
            outputfile5= "/home/jwren/Desktop/HYCOM100m8kmRun2/HYCOM45pld50n100m_" + str(startsite) + "_daily_1.txt"
